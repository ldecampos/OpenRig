...
_SPLIT_TEXT_WORDS_RE = re.compile(r'[A-Z]+(?=[A-Z][a-z])|[A-Z][a-z0-9]+|[A-Z]+|[a-z]+|[0-9]+')
...
def to_snake_case(text: str, remove_digits: bool = False) -> str:
    ...
    snake_case = '_'.join(snake_case_list)
...
def to_kebab_case(text: str, remove_digits: bool = False) -> str:
    ...
    kebab_case_list: list[str] = []
    for word in split_words:
        kebab_case_list.append(word.lower())
    kebab_case = '-'.join(kebab_case_list)
...
def split_text(text: str) -> list[str]:
    """Splits text into individual words, handling camelCase, PascalCase, and delimiters like _ and -.
...
def clean_txt(text: str, replace_with: str = '_') -> str:
    """Cleans a string by replacing illegal characters and ensuring it doesn't start with a digit.
...
def swap_substrings(text: str, mapping: dict[str, str] | None = None) -> str:
    ...
    # Sort keys by length descending to match longest keys first (e.g. 'Left' before 'L')
